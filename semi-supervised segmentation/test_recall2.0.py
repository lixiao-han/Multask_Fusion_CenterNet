...
for file in files:
    ...
    tree = ET.parse(file)
    root = tree.getroot()
    pyflow_bgsub_name = os.path.join('/store/datasets/UA-Detrac/fgmask1', file_name)
    num_total = 0
    ...
    for frame in root.findall('.//frame'):
        ...
        if num % 10 == 0:
            ...
            if len(boxes) == 0:
                ...
                py_path = os.path.join(pyflow_bgsub_name, 'img' + index + '.png')
                binary_pred_array = np.array(Image.open(py_path))
    ...
    lllbbb = recall_total/num_total
    print(file_name + '的总召回率为：' + str(recall_total/num_total))
    if lllbbb > 0.63:
        print(file_name + ',' )
        useable += 1
print('可用的视频文件夹有' + str(useable) + '个')
