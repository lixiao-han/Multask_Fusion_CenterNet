...
class IDAUp(nn.Module):
    ...
    def forward(self, layers):
        ...
        for i in range(1, len(layers)):
            ...
            x = node(torch.cat([x, layers[i]], 1)) #这里将x和layers[i]两个张量按通道进行拼接（是拼接不是相加！！！与pose_dla_dcn不同）
            y.append(x)                          #至于第二个参数为什么是1，是因为x和layers[i]都是4维张量，第一维为batch_size（下标0），所以1表示按通道拼接
        return x, y
