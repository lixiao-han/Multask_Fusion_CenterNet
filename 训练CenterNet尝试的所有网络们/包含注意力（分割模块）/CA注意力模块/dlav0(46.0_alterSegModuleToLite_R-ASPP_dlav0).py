...
class IDAUp(nn.Module):
    def __init__(self, node_kernel, out_dim, channels, up_factors):
        ...
        for i in range(1, len(channels)): #参考range函数的语法：i in range（a，b）则i取值从a到b-1，所以当len（channels）=2时，i只取1（循环一次）
            node = nn.Sequential(
    ...
    def forward(self, layers):
        ...
        for i in range(1, len(layers)):
            ...
            lll_LXH = torch.cat([x, layers[i]], 1)  #这里将x和layers[i]两个张量按通道进行拼接（是拼接不是相加！！！与pose_dla_dcn不同）
            x = node(lll_LXH)                    #至于第二个参数为什么是1，是因为x和layers[i]都是4维张量，第一维为batch_size（下标0），所以1表示按通道拼接
            y.append(x)
        return x, y
...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads,
        ...
        scales = [2 ** i for i in range(len(channels[(self.first_level-1):]))]
        self.dla_up = DLAUp(channels[(self.first_level - 1):], scales=scales)
        cccLXH = channels[self.first_level]
    ...
    def forward(self, x):
        x = self.base(x)
        x = self.dla_up(x[(self.first_level-1):])
        ret = {}
        seg_name = 'seg'
        y_1 = self.seglayer1(x[1])
        y_2 = self.seglayer2(x[1])
        y_3 = self.seglayer3(x[0])
        ...
        for head in self.heads:
            if not head == seg_name:
                if head == 'hm':
                    attention = nn.MaxPool2d(4)(ret[seg_name])
                    y = x[1] * attention
                    yy = self.att(y)
                    yyy = self.__getattr__(head)(yy)
                    ret[head] = yyy
                else:
