...
#Conv2d+CA 2023.03.20LXH
class DoubleConvLXH(nn.Module):
    """(convolution => [BN] => ReLU) * 2"""
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.double_conv = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            CoordAtt_1(in_channels, out_channels)
        )
...
#up+CA 2023.03.20LXH
class InPlaceUpLXH(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(InPlaceUpLXH, self).__init__()
        self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.conv = DoubleConvLXH(in_channels, out_channels)
...
class segmentation_module(nn.Module):
    def __init__(self, curr_dim, out_dim, **kwargs):
        ...
        self.up6 = InPlaceUpLXH(64, self.n_classes)
...
class U_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        e2 = self.Maxpool1(e1) #self.Maxpool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        e2 = self.Conv2(e2) #self.Conv2 = conv_block(filters[0], filters[1])
