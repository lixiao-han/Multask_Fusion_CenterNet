...
def main(opt):
  ...
  test_writer = SummaryWriter(log_dir="runs33.0_ModifyDla_pose's+toTorch.cat/test")
  test_half_writer = SummaryWriter(log_dir="runs33.0_ModifyDla_pose's+toTorch.cat/test_self")  #写错了，把half记成了self，只能将错就错下去
  test_easy_writer = SummaryWriter(log_dir="runs33.0_ModifyDla_pose's+toTorch.cat/test_easy")
  test_medium_writer = SummaryWriter(log_dir="runs33.0_ModifyDla_pose's+toTorch.cat/test_medium")
  test_hard_writer = SummaryWriter(log_dir="runs33.0_ModifyDla_pose's+toTorch.cat/test_hard")
  train_writer = SummaryWriter(log_dir="runs33.0_ModifyDla_pose's+toTorch.cat/train/loss")
  train_hmloss_writer = SummaryWriter(log_dir="runs33.0_ModifyDla_pose's+toTorch.cat/train/hmloss")
  #train_segloss_writer = SummaryWriter(log_dir="runs33.0_ModifyDla_pose's+toTorch.cat/train/segloss")
  val_writer = SummaryWriter(log_dir="runs33.0_ModifyDla_pose's+toTorch.cat/val")
  best = 0.65
  best_half = 0.8
  best_easy = 0.8
  best_medium = 0.7
  ...
  for epoch in range(start_epoch + 1, opt.num_epochs + 1):
    ...
    opt.model_LXH = model
    mAP_LXH, mAP_LXH_half, mAP_LXH_easy, mAP_LXH_medium, mAP_LXH_hard = test.prefetch_test(opt)
    opt.keep_res = False
