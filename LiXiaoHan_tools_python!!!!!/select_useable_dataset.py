...
# 读取原始标注文件
with open("/store/datasets/UA-Detrac/COCO-format/train_b.json", "r") as f:
    data = json.load(f)
...
for ann in data["annotations"]:
    img_id = ann["image_id"]
    img = next((i for i in new_data["images"] if i["id"] == img_id), None)
    if img is not None:
        new_data["annotations"].append(ann)

# 输出新的标注文件
with open("/store/datasets/UA-Detrac/COCO-format/train-1-on-5_b.json", "w") as f:
